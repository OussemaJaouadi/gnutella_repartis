	...
	def serve(self, choice: str) -> None:
		...
		if choice == "QUER":
			...
			while True:
				...
				try:
					...
					if 1 <= index <= len(files):
						...
						UI_colors.print_red(f'Index chosen must be in the correct range: 1 - {len(files)}.\n')
				except ValueError:
					UI_colors.print_red(f'Your choice must be a valid one: number in range 1 - {len(files)} expected.\n')
			...
			for count, neighbour in enumerate(AppData.get_neighbours(), start=1):
				UI_colors.print_blue(f'{count}] {AppData.get_peer_ip4(neighbour)} {AppData.get_peer_ip6(neighbour)} {str(AppData.get_peer_port(neighbour))}')
			...
			while True:
				...
				try:
					...
					if 1 <= index <= len(neighbours):
						...
						UI_colors.print_red(f'Index chosen must be in the correct range: 1 - {len(neighbours)}.\n')
				except ValueError:
					UI_colors.print_red(f'Your choice must be a valid one: number in range 1 - {len(neighbours)} expected.\n')
